...
/**
 * build linux mtp option for 2.6.27 kernel
 *  char * optionbuf------buffer for building option
 *  int len---------------buffer length
 * return:
 * 	0-----success
 *  todo: to support mult device
 */
int build_linux_mtdoption(char * optionbuf, int len)
{
	struct list_head *dentry, *pentry;
	struct part_info *part;
	struct mtd_device *dev;
	...
	list_for_each(dentry, &devices) {
		dev = list_entry(dentry, struct mtd_device, link);
		sprintf(optionbuf, "mtdparts=%s:", dev->id->mtd_id);
		printf("num_parts = %d\n", dev->num_parts );
		if( dev->num_parts > 0 ){
			part_num = 0;
			list_for_each(pentry, &dev->parts) {
				...
				memset(temp, 0x0, 64);
				if( part ){
					if( part->size >= 1024*1024 ){/* unit = 'M'; */
						sprintf(temp, "%dM(%s)", part->size/(1024*1024), part->name);
						printf("partitions %s\n", temp);
					} else {/* unit = 'k'; */
						sprintf(temp, "%dK(%s)", part->size/(1024), part->name);
						printf("partitions %s\n", temp);
					}
					part_num ++;
					...
					strcat(optionbuf, temp);
				}
			}
		}else{
			printf("num_parts = %d, no partitions defined\n", dev->num_parts );
			return -1;
		}
	}
	return 0;
 ...
 * @return 0 on success, 1 otherwise
 */
int do_jffs2_mtdparts_akaeyukeban(void)
{
